...
## 23. Fees-module installment sub-form responsiveness (2026-06-25, local — UNCOMMITTED)

While share-testing as Admin over the local Cloudflare Tunnel, the owner found the **Edit fee schedule** dialog's **installment sub-form "so poorly rendered"** and asked to fix it accounting for Tailwind responsive breakpoints. Single-file working-tree change in `resources/js/pages/admin/fees/Index.vue` (not yet committed/PR'd; `main` still at `9f23c08`).
