...
## 26. #80 execution — backend Tasks 1–5 SHIPPED on branch, UI + gate + docs pending (2026-07-04)

Subagent-driven execution of `plan/documents-requested/plan.md` began (fresh implementer per task + per-task spec/quality review, ledger in `.superpowers/sdd/progress.md` — git-ignored scratch). **Backend complete: Tasks 1–5 done, each review-clean with zero Critical/Important findings.** Paused at owner request after Task 5; **resume at Task 6 (SAO Review page UI)**.

- **Commits on `feat/documents-requested-response`** (all off §25's docs commits, all Pint-clean, TDD RED→GREEN evidenced, trailer verified):
  - `30045e4` T1 — per-document review state: 4 columns on `application_documents` (original migration edited in place), `ApplicationDocumentStatus` enum, model casts + `reviewedBy()`, factory `accepted()`/`rejected(?string)` states. One justified deviation: factory `definition()` also gained `'status' => 'pending'` because `create()` doesn't re-read DB defaults — TDD surfaced it, reviewer verified all 6 other factory call sites unaffected.
  - `119665b` T2 — SAO `POST …/documents/{document}/accept|reject` (scoped bindings), `ReviewApplicationDocument` action (both rows `lockForUpdate`, terminal refusal on key `status`, `DocumentAccepted`/`DocumentRejected` audits), shared `ResolvesDocumentsRequested::flipToSubmittedWhenResolved()` trait, 3 new `AuditAction` cases, SAO `show()` payload + `status`/`review_notes`. 8 tests.
  - `8b184ad` T3 — triage guard: `DocumentsRequested` needs ≥1 rejected doc (ValidationException on `status`), `notes` relaxed to `nullable|string|max:5000`; 2 triage tests replaced + 2 added (file now 8).
  - `eeee6e1` T4 — entry email: `ApplicationDocumentsRequested` event → `SendDocumentsRequestedNotification` (ShouldQueue) → `ApplicationDocumentsRequestedMail` (Queueable, NOT ShouldQueue — decision-mail parity) + markdown view listing rejected docs/reasons; dispatched via `DB::afterCommit` on every entry. +2 tests (fires per entry incl. re-entry; silent on other transitions).
  - `c7dbc1d` T5 — applicant `POST applications/{application}/documents/{document}` (`application.documents.replace`), `ReplaceRejectedDocument` action (store-file-before-transaction + rollback cleanup + old-file delete after commit per AUD-009; in-place row update; review metadata reset to pending; `DocumentResubmitted` audit; auto-flip via shared trait, `submitted_at` preserved), `StoreApplicationRequest` mime/size constants `private→public` reused by `ReplaceDocumentRequest`, applicant `show()` payload + `status`/`review_notes`, ownership `abort_if` in controller. 7 tests; **full Unit+Feature regression 722/722 green, zero unrelated fixups.**
- **Minor findings parked for the final whole-branch review** (none blocking): no test on the `Pending`-decision `InvalidArgumentException` guard in `ReviewApplicationDocument`; no test for accepting a non-rejected doc while another rejected remains; mail blade em-dash indent under `@if` unverified visually (Mail::fake only); listener doc comment paraphrases its sibling; **replace route has no `throttle:lookups` unlike its GET siblings (plan-mandated shape — decide at branch review)**; brief-mandated overlap between the new "still allows other interim transitions" triage test and the file's first test.
- **Remaining:** T6 SAO Review.vue (Wayfinder regen → statusDisplay maps → Review tag column + Accept/Reject + reject dialog), T7 applicant Show.vue (banner + status column + per-row FileUpload replace), T8 full gate, T9 docs/ADR-0023/context §/PR closing #80. Final whole-branch review before finishing-a-development-branch.
- **Resume:** re-enter superpowers:subagent-driven-development; the ledger marks Tasks 1–5 complete — do NOT re-dispatch them; next dispatch is Task 6 (`scripts/task-brief plan/documents-requested/plan.md 6`, base `c7dbc1d`).

## 27. #80 execution — UI + gate + docs complete, PR opened to close #80 (2026-07-05)

Resumed the subagent-driven run at Task 6 and carried it to the finish: both UI layers, the full quality gate, and the docs/ADR sync all shipped on `feat/documents-requested-response`, each task fresh-implementer + per-task spec/quality review, every review clean (zero Critical/Important).

- **Commits (continuing the §26 chain, all trailer-verified):**
  - `076951e` T6 — SAO document-review UI on `resources/js/pages/sao/applications/Review.vue`: Wayfinder regen for the accept/reject routes, `APPLICATION_DOCUMENT_STATUS` map + `applicationDocumentStatusLabel`/`Severity` helpers in `resources/js/lib/statusDisplay.ts`, a Review status column (Tag + notes), per-row Accept/Reject buttons gated on `!is_terminal` (+ per-status hide), and a reject dialog (optional-notes Textarea via `useForm`). The obsolete `triageNotesRequired` gating was removed (triage notes now optional). Frontend gate green.
  - `10351ba` T7 — applicant response UI on `resources/js/pages/applicant/applications/Show.vue`: a warning `Message` banner when awaiting response, a Status column (Tag + red rejection notes), and a per-rejected-document `FileUpload` (basic, custom-upload, pdf/jpg/jpeg/png ≤ 8 MB) posting to the replace route with `forceFormData` + `preserveScroll`. One lint-driven deviation from the brief snippet: a merged type+value import split into two top-level imports (`import/consistent-type-specifier-style`, matching `sao/applications/Index.vue`). Frontend gate green.
  - `f0bba8a` T8 — full quality gate: `vendor/bin/pint --format agent` (auto-fixed 7 **unrelated** pre-existing-drift files — 5 test EOF-newlines, `plan/report/seed_demo.php` imports, `ExampleTest`; committed as `chore(gate)`), **Pest 722/722 Unit+Feature**, `npm run build` (no chunk-size warning), `types:check`, `lint:check`, `migrate:fresh --seed` — all green. (Env note: the gate runner had to start local MySQL + Redis via Laragon binaries; `CACHE_STORE=redis` means `DatabaseSeeder`'s `ReferenceDataCache::flush()` needs Redis up.)
  - `docs:` T9 — this doc sync. `docs/routes.md` count 122→125 + the 3 new rows (SAO accept/reject, applicant replace); `docs/modules/admissions.md` (data-model row + `ApplicationDocumentStatus`, route tables, §5.2 corrected — triage-to-`DocumentsRequested` now needs ≥1 rejected doc not required notes, and it *does* email; new §5.6 round-trip flow; §6.1/6.2 event+audit rows; §7 tests; §8 file map; §2 applicant ability; footer); both guides (`applicant.md` new "Respond to a documents request" walkthrough + status/notification updates; `sao.md` document accept/reject + corrected triage rule + notification + FAQ); plus honest spillover the routing table demanded — `docs/data-model.md` (4 new `application_documents` columns + enum entry), `docs/modules/notifications.md` (four→five transactional mailables + side-effects + test rows), `docs/index.md` (ADR count 22→23); and **ADR-0023** `docs/adr/0023-structured-application-document-review.md` (+ README index row).
- **All §26 Minor findings still parked for the final whole-branch review**, now joined by T6/T7's: `acceptDocument`/replace `FileUpload` have no in-flight double-submit guard; reject-dialog error-text classes differ from the triage sibling (brief-mandated snippet). None blocking; the replace route's missing `throttle:lookups` is captured in ADR-0023's As-built section.
- **Next:** final whole-branch review (most-capable model, `scripts/review-package $(git merge-base main HEAD) HEAD`, fed the parked Minor list) → superpowers:finishing-a-development-branch → PR closing #80 → wait for 4 CI checks → squash-merge + delete branch + fast-forward local `main`.

## 28. #81 results & dispute notifications — shipped on branch, PR opened (2026-07-05)

Second omissions-audit backlog item (§24 finding 2). Brainstormed → spec (`plan/results-notifications/design.md`, `5ebca61`) → 5-task TDD plan (`plan/results-notifications/plan.md`, `ae4a6b5`) → subagent-driven execution on `feat/results-notifications` (off `main` @ `d7a4aab`, i.e. after #80 merged). Two queued multi-channel Notifications on the exact #12 chain (Action → `DB::afterCommit` Event → queued Listener → `Notification::send` → `ShouldQueue` Notification, `via` mail+database), **nudge-only** (no scores). Owner-approved design decisions: publish notifies **only the just-published students** (`student_profile_id`s captured at publish time, so a re-publish notifies only the newly-published); dispute notifies the single disputing student on **terminal outcomes only**.

- **Commits (all trailer-verified, each task fresh implementer + per-task spec/quality review, all review-clean zero Critical/Important):**
  - `1f9a875` T1 — results-published: `CourseResultsPublished` event, `SendCourseResultsPublishedNotification` listener, `CourseResultsPublishedNotification` (`via` mail+database, `toArray.type='course_results_published'`), `mail.course-results-published` blade, `PublishCourseResults` afterCommit dispatch. 2 tests.
  - `e0b36c3` T2 — dispute-reviewed: `ResultDisputeReviewed` event, `SendResultDisputeReviewedNotification` listener, `ResultDisputeReviewedNotification` (`toArray.type='result_dispute_reviewed'`, carries status + resolution_notes), `mail.result-dispute-reviewed` blade, `ReviewResultDispute` afterCommit dispatch **inside the terminal `if`**. 3 tests. (Haiku implementer wrote its report to a corrupt Windows path — code committed correctly; reviewer verified against the diff; stray file cleaned.)
  - `c1c9730` T3 — student feed render cases in `resources/js/pages/dashboards/Student.vue`: `NotificationData` widened to a discriminated union + `notificationTitle`/`notificationIcon`/`notificationDetail` helpers; two lucide icons; the `import/consistent-type-specifier-style` import-split deviation (matches `StatCard.vue`). Frontend gate green.
  - T4 — full gate green, no commit needed: **Pest 728/728** Unit+Feature (2760 assertions), build no chunk-size warning (largest 446 kB), types+lint+pint clean.
  - `docs:` T5 — this sync: `docs/modules/notifications.md` (§5.1.1 new multi-channel family + §6 trigger rows + §8 tests; "first and only" corrected), `docs/modules/course-management.md` (§5.4/§5.5 dispatch notes, §7.2 rewritten — the old "publish/dispute mail was not shipped" claim removed, §8 tests, §9 file map), `docs/guides/student.md` (Notifications section + dispute-status note). No routes/schema/ADR (falls under ADR-0019).
- **Parked Minor findings for the final whole-branch review:** T2 dispatches `$dispute` not `$dispute->fresh()` unlike sibling actions (plan-mandated snippet; harmless — no relation mutated); T2 re-derives course in both `toMail`+`toArray` (style); T3's three helper switches rely on TS narrowing exhaustiveness without a `default` (latent trap if a 5th type is added without touching all three; matches the pre-existing `notificationTitle` pattern); no isolated `toArray`-shape test.
- **Next:** final whole-branch review → finishing-a-development-branch → PR closing #81 → 4 CI checks → squash-merge + delete + fast-forward local `main`.

## 29. #82 remove the Draft dead state — shipped on branch, PR opened (2026-07-05)

Third omissions-audit backlog item (§24 finding 3). Brainstormed → owner chose **delete the unreachable `Draft` state** (not build save-as-draft) → spec (`plan/remove-draft-state/design.md`, `34ec0e8`) → 2-task plan (`plan/remove-draft-state/plan.md`, `6ba583d`) → subagent-driven execution on `chore/remove-draft-state` (off `main` @ `d8fa04a`). `ApplicationController::store()` already created every application as `Submitted`; nothing ever persisted a `Draft`, so this was a behavior-preserving deletion of dead surface.

- **Task 1 (`658b55a`, sonnet implementer + sonnet review, clean):** removed the `Draft` enum case; dropped its `OPEN_STATUSES` membership (now `[...INTERIM_STATUSES]`, kept as a semantic alias); removed the unreachable `Draft → Submitted` branch in `canTransitionTo()` (interim path → `return true`, signature kept per spec); flipped the migration `status` default `Draft`→`Submitted`; removed the SAO `priorApplications` `whereNotIn(Draft)` filter; flipped the `ApplicationFactory` default to `Submitted` + `submitted_at => now()` (kept `submitted()` state); removed the `draft` line from the `APPLICATION_STATUS` map only; deleted the two `it('refuses to … a draft application')` cases in Decide/Triage tests. **Suite 729/729** (731 − 2 removed), pint + build/types/lint green, 0 factory-fallout fixes. One extra fix the reviewer verified justified: `SaoDashboardTest` asserted a `statusCounts.draft` key that can't exist once `Draft` leaves `ApplicationStatus::cases()`.
- **Task 2 (`docs:`):** **ADR-0024** (applications born Submitted; save-as-draft rejected — nullable schema + draft-file lifecycle disproportionate for a one-time submit artifact) + README index; `docs/data-model.md` + `docs/modules/admissions.md` (§3 enum, `OPEN_STATUSES` wording, `canTransitionTo` prose) drop `Draft`; `docs/index.md` ADR count 23→24; this §29.
- **Parked Minor for the final whole-branch review:** the now-vestigial `canTransitionTo(ApplicationStatus $next)` parameter (unconsulted; plan-mandated keep-signature; no CI gate catches it — PHPStan would); the `submitted()` factory state is now redundant with the default (left per instruction).
- **Next:** final whole-branch review → finishing-a-development-branch → PR closing #82 → 4 CI checks → squash-merge + delete + fast-forward local `main`.

## 30. #83 retire the ability gates — shipped on branch, PR opened (2026-07-05)

Fourth (final) omissions-audit backlog item (§24 finding 4) + the ADR-0022 follow-up. Owner chose **retire all 8** ability gates (over wiring the 5 uninvoked ones, or the issue's retire-5-keep-3) after the redundancy analysis: every gate's role set is identical to the roles of the route group its endpoints sit behind, so the gates only restate the `role:*` middleware — the real authorization is `role:*` route middleware + per-resource ownership. Brainstormed → spec (`plan/retire-ability-gates/design.md`, `e4019c1`) → 2-task plan (`plan/retire-ability-gates/plan.md`, `5f87e4c`) → subagent-driven execution on `chore/retire-ability-gates` (off `main` @ `17b0f4b`).

- **Task 1 (`11ed6df`, sonnet implementer + sonnet review, clean, zero findings):** removed `ABILITIES` + `configureGates()` + boot call + unused `Gate`/`RoleName` imports from `AppServiceProvider` (kept `User` — audit listeners use it); removed the 3 `Gate::authorize` calls from `Sao\CourseController` + the 1 from `Lecturer\CourseSessionController` (kept `authorizeOwnership()`); deleted `AbilityGatesTest` + `ViewAuditLogGateTest` (no replacement — `AdminAuthorizationTest` already asserts 403 for every non-admin on `GET /admin/audit-logs` + `AuditLogIndexTest` the admin 200). **Behavior-preserving** — reviewer independently verified route middleware enforces the identical role sets (`EnsureUserHasRole` == the deleted `Gate::define` closures) and denial paths are tested (`CoursePlanApprovalTest`/`PublishCourseResultsTest`/`MarkAttendanceTest`). Suite **682/682** (729 − the deleted gate-definition tests' dataset cases), all gates green. A commit-security hook flagged the removed `Gate::authorize` as "Missing Authorization" (HIGH/MED) — **confirmed false positive** (route-group middleware already enforces the same roles; owner-approved with tradeoffs surfaced during design).
- **Task 2 (`docs:`):** **ADR-0025** (retire the gates; role middleware + ownership is the single model) **supersedes ADR-0022** (status flipped); README index; `security.md` §2.1 repurposed + §3.4 fixed; `architecture.md` gate table removed; `index.md` descriptions + ADR count 24→25; module sweep (`course-management` 3 wired gates → middleware in tables/mermaid/prose/test-cells, `admin-user-management` view-audit-log, `payments` validate-payment, `admissions` process/decide, `exam-gating`/`notifications` mentions); `ADR-0012` view-audit-log credit; plus iterative-sweep catches (`testing.md` deleted-test rows + dataset example, `onboarding.md` "add a gated ability" how-to, deleted-test rows in admissions/course-management file maps). ADR-0022 body left intact as historical record.
- **Next:** final whole-branch review → finishing-a-development-branch → PR closing #83 → 4 CI checks → squash-merge + delete + fast-forward local `main`. **This completes all four omissions-audit backlog items (#80–#83); only #71 (transcripts) remains.**

## 31. #71 student transcripts — DESIGNED → BUILT → MERGED (2026-07-06/07)

The last open backlog item (B16). Brainstormed **5 locked decisions** (sensible-default template; **4.0 GPA/CGPA**; **snapshot-at-issue** verification; **no** fee/status gating; add a minimal **SAO Students index** since no per-student view existed) → spec (`plan/transcripts/design.md`, `47607b1`) → 8-task TDD plan (`plan/transcripts/plan.md`, `c057d3a`) → **subagent-driven execution** on `feat/student-transcripts` (off `main` @ `403f2ae`), fresh implementer + per-task spec/quality review each.

- **T1** `549dd0b` + fix `b44c391` — `TranscriptService` (Published-only snapshot, `GRADE_POINTS` 4.0 A=4…F=0, credit-weighted semester GPA + cumulative CGPA, `contentDigest` meta-stripped). Reviewer **Critical**: `sortBy([closures])` routes to `sortByMany` 2-arg comparator (Laravel 13.7 `Collection.php:1639`) → single-param closures scrambled the sort → fixed with `->sort()` array-spaceship comparator + an out-of-order regression test.
- **T2** `0c41b64` — immutable `Transcript` model + `transcripts`/`transcript_sequences` migrations + factory. HMAC over `number|issued_at_iso|content_digest`; per-year `TRN-{year}-{00001}` via `lockForUpdate`; `booted()` throws on update/delete. Faithful mirror of `SchoolReceipt`.
- **T3** `16e892c` — `IssueTranscript` action: `null` on no published results; **dedupe** by `(student_profile_id, content_digest)` OUTSIDE the txn (non-unique index → app-level check); mint + audit INSIDE `DB::transaction` (allocation + create + one `AuditAction::TranscriptGenerated`).
- **T4** `9a67173` — public `transcripts.verify` (`throttle:lookups`) + standalone `transcripts/Verify.vue`; **no existence oracle** (unknown/forged/tampered all `valid:false`/`transcript:null`, payload built only inside `$valid ? [...] : null`).
- **deps** `a8a19ca` (orchestrator) — installed **`mpdf/mpdf ^8.3`** (dist not source — git clone timed out at 300s). **DROPPED `simplesoftwareio/simple-qrcode`** (pins `bacon/bacon-qr-code ^2.0`, conflicts with Fortify's `^3.0`) → QR rendered **directly via the already-installed bacon** (`Writer`+`ImageRenderer`+`SvgImageBackEnd`+`RendererStyle(120,0)`, strip `<?xml?>` prolog). CI `tests.yml` both setup-php steps: `extensions: sockets, gd, mbstring`.
- **T5** `7c93484` — `TranscriptPdfRenderer` (mpdf + bacon QR) + `resources/views/pdf/transcript.blade.php` (reads ONLY the snapshot → re-renders snapshot-at-issue).
- **T6** `0ec31dd` — student self-service: `Student\TranscriptController` (own profile only, **no id in URL → no IDOR**), `student.transcript` route, `hasTranscript` prop, results-page button. **GOTCHA:** `wayfinder:generate` MUST use `--with-form` here (`vite.config.ts formVariants:true`) or it strips `.form` variants app-wide.
- **T7** `0c48f02` — SAO students index: `Sao\StudentController` (index + any-student transcript), `sao.students.index/transcript` routes (**`role:sao,admin` group only, no gate** — ADR-0025; Lecturer→403 auto), PrimeVue DataTable page, `AppSidebarNav` "Students" item.
- **T8** `7805a77` — docs (`docs/modules/transcripts.md`, **ADR-0026** GPA / **0027** mpdf / **0028** snapshot-verification, refs, `security.md §4.1`) + `TranscriptDemoSeeder`. **Executed directly by the controller** — the implementer subagent hit an account session-limit twice (wrote nothing); docs authored + verified against shipped source; `migrate:fresh --seed` clean on SQLite AND real MySQL; end-to-end demo issue → `TRN-2026-00001`, cgpa 2.76, verifies TRUE, pdf `%PDF`.
- **FINAL whole-branch review (opus)** caught a **CRITICAL every SQLite-based per-task review structurally missed**: MySQL's native JSON column **reorders object keys** on storage → `content_digest` recomputed from the re-read snapshot at verify time diverged from issue time → `verifies()`=FALSE for **every genuine transcript on MySQL** (SQLite preserves key order, so all 705 tests stayed green). **FIXED** `f9b1fa9` — recursive `ksort` in `contentDigest` (key-order invariant) + a DB-agnostic regression test; reproduced then re-verified `verifies()`=TRUE on real MySQL. → [[reference-mysql-json-key-order-digest]]. All accumulated per-task Minors triaged as follow-up (none blocked merge).
- **MERGED:** PR **#90** squash `e1cd180`; issue **#71 auto-closed**; branch deleted; local `main` synced; **suite 705/705**; ADR count **25→28**. **The entire feature backlog is now shipped — no open backlog issues remain.**

## 32. #71 transcript review follow-ups (#91–#94, 2026-07-07)

The non-blocking T7 Minors from #71's whole-branch review, each own branch → PR → squash-merge (all 4 CI green, local `main` fast-forwarded each time):

- **#91** `48517f2` — `sao/students/Index.vue`: replaced the hand-rolled `v-html` pagination with PrimeVue DataTable `lazy`/`paginator`/`@page` inside a `Card` (matches the sibling SAO/admin index pages); `Sao\StudentController` gained a validated `rows` param (15/25/50).
- **#92** `c3ccb32` — students index **Programme** column now shows department + degree (`degreeLabel`); + a test covering the previously-untested `orWhereHas('user', name)` search branch.
- **#93** `09d32ae` — the **transcript snapshot** `programme` is now `"{department} ({degree})"` (new **`DegreeProgram::label()`**, mirrors the frontend `degreeLabel` map). Affects **newly issued** transcripts only — existing immutable records keep their signature (re-verified `verifies()`=TRUE on MySQL). Suite **707/707**.
- **#94** `317273f` — docs sync for the programme change (`docs/data-model.md` `transcripts.programme` column note + `docs/modules/transcripts.md` snapshot-shape note). Docs-only.

## 33. Presentation deck (#95, 2026-07-07)

A **15-slide academic-defense PowerPoint** under `plan/deck/` (sibling to `plan/report/`), on owner request ("as creative as possible, figures + charts"). Brainstormed → audience locked = **academic defense**; jury are **StarUML fans** → matched classic UML notation.

- **`build_deck.py`** (python-pptx) assembles 15 slides in an emerald master layout (kicker/title/rule, footer `NN / 15`); helpers `figslide`/`twofig`/`figbullets`/`stat_tiles`/`title_slide`/`results_slide`/`conclusion_slide`. Slide 1 carries `«placeholders»` (owner fills in PowerPoint).
- **`make_charts.py`** (matplotlib) → 9 **dataviz-skill-validated** brand charts (`charts/`): emerald ramp + validated categorical `#047857/#1D4ED8/#B45309`; the transcript GPA chart uses REAL demo data (Sem1 3.10, Sem2 2.29, CGPA 2.76).
- **`uml/*.puml`** — 9 StarUML-style diagrams via **PlantUML** (class, use-case with system boundary + `«include»`, crow's-foot ER, 3 sequences w/ activation bars + `alt`, swimlaned activity, state machine, component/deployment); shared skin `uml/_style.puml`; **Smetana layout engine (no graphviz needed)**; Java 21. Reflects the CURRENT system (no `Draft`, has `Transcript`).
- Reuses the real screenshots from `plan/report/screenshots/`. **QA via PowerPoint COM** (`pptx → pdf` `SaveAs(...,32)`; `pptx → per-slide PNG` `Export(dir,"PNG",1920,1080)`) — the Read tool can't render PDF (no poppler).
- **`.gitignore`** excludes `tools/plantuml.jar` (22 MB build tool) + `build/png/` (QA renders); rendered diagram/chart PNGs + the `.pptx`/`.pdf` ARE committed. `README.md` documents rebuild.
- **MERGED:** PR **#95** squash `b7d54ab`; 34 files tracked under `plan/deck/`. → [[project-presentation-deck]].
