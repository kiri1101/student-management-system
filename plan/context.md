...
- **RESUME → D3** (ADR folder — fan out `adr-writer` over the ~20 locked decisions in `plan/documentation/plan.md`; fold in the two live drifts above — "gates defined-but-not-invoked" and "single-role-staff vs multi-role pivot" — each wants an ADR). Then D4 guides (6× `docs-user-guide-writer`) · D5 PHPDoc pass (`laravel-backend-drafter`, runs Pint+Pest) · D6 README + cross-link. Branch `docs/initiative-68`, not pushed; PR at the end.
