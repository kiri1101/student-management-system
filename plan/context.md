...
- Payment validation + tamper-proof school receipts (signed QR, verification endpoint).
- Tuition deferral request/approval flow + facility/exam access gating.
...
**Next up — #6 Payment validation + tamper-proof school receipts (backlog B1):** design agreed 2026-06-13, full plan persisted at **`plan/payments/plan.md`**. Three phases (P1 admin fee config → P2 payment submission + accountant validation → P3 HMAC-signed receipts + public verification, the latter also closing #16). Key decisions: slip-upload is the lost-slip fix; Admin owns fee config; HMAC signatures verified server-side; money is integer XAF; #8 access-gating stays out of scope; flat fee schedule per (offering, level, academic_year). **Implementation begins at P1 next session.** Remaining backlog after #6: #8 deferral, #11 course mgmt, #12 lecturer-absence, #18 notif strategy, #20 multi-role switcher (B7, held), #22 shadcn→PrimeVue (opportunistic), #30 CSV import, #37 Mailtrap walkthrough (manual), #39 reference caching (blocked).
