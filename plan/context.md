...
- **RESUME → D4** (6 per-role user guides — fan out `docs-user-guide-writer` over applicant/student/sao/accountant/lecturer/admin; ground each in the role's actual routes + Inertia pages). NOTE the Write-denial above: expect to author guide files from the drafters' returned content. Then D5 PHPDoc pass (`laravel-backend-drafter`, runs Pint+Pest — first phase that touches `app/` + the real gate) · D6 README + cross-link + wire `docs-refresh` into the gate. Branch `docs/initiative-68`, not pushed; PR at the end.
