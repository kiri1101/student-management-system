...
**STATUS:** Substantial feature backlog complete; #39 shipped, #20 closed not-planned. **#22** shadcn→PrimeVue — ✅ login + register migrated to PrimeVue via the frontend-redesign pilot (§18, PR #63). Remaining open issue: **#37** invite-flow Mailtrap walkthrough (manual QA, needs a human).
...
- **Next (DEFERRED — "proceed later"):** roll the brand into the staff **feature pages** (SAO review queues, course management, payments, deferrals, results, admin reference/users tables) — dashboards are done, feature pages still wear the starter look. See memory [[project-frontend-redesign]] for the live resume point.
