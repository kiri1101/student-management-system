...
- **RESUME → D6** (final phase): root `README.md` + cross-link the `/docs` tree + link docs from both `CLAUDE.md`s + wire `docs-refresh` into the per-feature gate + final verification pass, then open the PR for #68. Optional follow-up tracked from D3: tighten `security.md §2.1/§3.4` wording re: the `view-audit-log` gate (it's `role:admin` middleware, not the gate, that guards the endpoint). Branch `docs/initiative-68`, not pushed; PR at the end.
