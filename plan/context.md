...
- **Gotchas:** `eslint .` lints the whole repo → committing the prototype's `app.js` failed CI until `plan` was added to eslint `ignores`. `npm run format` reformats **all** of `resources/` — scope Prettier to changed files. PrimeVue `Password`/`InputText` put the `name` prop on the inner input via `$formName`, so Inertia `<Form>` serialization + Pest `fill()` both work.
...
- **RESUME → D2** (6 module docs, fan out 6× `docs-module-writer`), then D3 ADRs · D4 guides · D5 PHPDoc · D6 README.
