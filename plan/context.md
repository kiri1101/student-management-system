...
- **NEXT (needs user go-ahead — outward-facing): open the PR for #68.** Branch `docs/initiative-68` is committed but **not pushed**. Optional follow-up still open: tighten `security.md §2.1/§3.4` wording re: the `view-audit-log` gate (it's `role:admin` route middleware, not the ability gate, that guards the audit endpoint — 3 of 8 gates wired, per ADR-0022).
